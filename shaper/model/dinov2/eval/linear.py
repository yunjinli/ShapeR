...
# pyre-fixme[21]: Could not find module `dinov2.distributed`.
...
# pyre-fixme[21]: Could not find module `dinov2.data`.
from dinov2.data import make_data_loader, make_dataset, SamplerType

# pyre-fixme[21]: Could not find module `dinov2.data.transforms`.
from dinov2.data.transforms import (
    make_classification_eval_transform,
    make_classification_train_transform,
)

# pyre-fixme[21]: Could not find module `dinov2.eval.metrics`.
from dinov2.eval.metrics import build_metric, MetricType

# pyre-fixme[21]: Could not find module `dinov2.eval.setup`.
from dinov2.eval.setup import (
    get_args_parser as get_setup_args_parser,
    setup_and_build_model,
)

# pyre-fixme[21]: Could not find module `dinov2.eval.utils`.
from dinov2.eval.utils import evaluate, ModelWithIntermediateLayers

# pyre-fixme[21]: Could not find module `dinov2.logging`.
from dinov2.logging import MetricLogger

# pyre-fixme[21]: Could not find module `fvcore.common.checkpoint`.
...
def remove_ddp_wrapper(m: nn.Module) -> nn.Module:
    # pyre-fixme[7]: Expected `Module` but got `Union[Module, Tensor]`.
...
class LinearClassifier(nn.Module):
    ...
    def __init__(
        self, out_dim, use_n_blocks, use_avgpool, num_classes: int = 1000
    ) -> None:
        super().__init__()
...
class AllClassifiers(nn.Module):
    def __init__(self, classifiers_dict) -> None:
        super().__init__()
...
class LinearPostprocessor(nn.Module):
    def __init__(self, linear_classifier, class_mapping=None) -> None:
        super().__init__()
        self.linear_classifier = linear_classifier
        # pyre-fixme[8]: Attribute has type `Tensor`; used as `Optional[LongTensor]`.
...
def eval_linear(
    *,
    ...
    for data, labels in metric_logger.log_every(
        ...
        optimizer.zero_grad()
        # pyre-fixme[16]: `int` has no attribute `backward`.
        loss.backward()
        ...
        if iteration % 10 == 0:
            ...
            # pyre-fixme[16]: `int` has no attribute `item`.
