...
# pyre-fixme[11]: Annotation `Checkpointer` is not defined as a type.
class FSDPCheckpointer(Checkpointer):
    def save(self, name: str, **kwargs: Any) -> None:
        ...
        """
        # pyre-fixme[16]: `FSDPCheckpointer` has no attribute `save_dir`.
        # pyre-fixme[16]: `FSDPCheckpointer` has no attribute `save_to_disk`.
        if not self.save_dir or not self.save_to_disk:
            return

        data = {}
        # pyre-fixme[16]: `FSDPCheckpointer` has no attribute `model`.
        with FSDP.state_dict_type(self.model, StateDictType.LOCAL_STATE_DICT):
            data["model"] = self.model.state_dict()

        # data["model"] = self.model.state_dict()
        # pyre-fixme[16]: `FSDPCheckpointer` has no attribute `checkpointables`.
        ...
        # pyre-fixme[16]: `FSDPCheckpointer` has no attribute `logger`.
        self.logger.info("Saving checkpoint to {}".format(save_file))
        # pyre-fixme[16]: `FSDPCheckpointer` has no attribute `path_manager`.
